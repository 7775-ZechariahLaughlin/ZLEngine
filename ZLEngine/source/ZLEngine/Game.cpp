...
void Game::Run()
{
	if (!bIsGameOver) {
		...
		Graphics->EngineDefaultCam->GetTransforms().Location = Vector3(9.0f, 2.5f, 9.0f);
...
void Game::ProcessInput()
{
	...
	if (GameInput->IsMouseButtonDown(MouseButtons::RIGHT)) {
		...
		if (CurrentState == PlayerState::EditorMode) {
			...
			if (Graphics->EngineDefaultCam->GetTransforms().Location.y > 1.5f) {
				CameraInput += 3.0f * -CamDirections.Up;
			}
		}
	}
	...
	if (GameInput->IsKeyDown(SDL_SCANCODE_SPACE)) {
		...
		if (!Jumping) {
			for (int i = 0; i <= 6; i++) {
				...
				Graphics->EngineDefaultCam->CalculateMovementInput(CamDirections.Up, CurrentState);
			}
			Jumping = true;
			OnFloor = false;
		}
	...
	if (Graphics->EngineDefaultCam->GetTransforms().Location != PositionToMove) {
